...
<<<<<<< HEAD
	if(klen == DES3_KEY2_SIZE)//16å­—èŠ‚
		des3_set2key_enc( &ctx3, pkey );//æ ¹æ®é•¿åº¦è®¾ç½®key
	else if(klen == DES3_KEY3_SIZE)//24å­—èŠ‚
=======
	if(klen == DES3_KEY2_SIZE)//16×Ö½Ú
		des3_set2key_enc( &ctx3, pkey );//¸ù¾Ý³¤¶ÈÉèÖÃkey
	else if(klen == DES3_KEY3_SIZE)//24×Ö½Ú
>>>>>>> add all file
...
<<<<<<< HEAD
	//ch = 8 - nlen % 8;//å¯ä»¥è®¾ç½®è¡¥é½å†…å®¹ï¼Œå¸¸ç”¨0æˆ–0xFF
=======
	//ch = 8 - nlen % 8;//¿ÉÒÔÉèÖÃ²¹ÆëÄÚÈÝ£¬³£ÓÃ0»ò0xFF
>>>>>>> add all file
    for(i = 0;i < nlen;i += 8)
    {
...
<<<<<<< HEAD
    if(len > nlen)//ä¸è¶³8å­—èŠ‚è¡¥é½
=======
    if(len > nlen)//²»×ã8×Ö½Ú²¹Æë
>>>>>>> add all file
    {
		tmp = (unsigned char *)malloc(len);
        i -= 8;
...
<<<<<<< HEAD
//	len = MyStrToHex("3F12E7C02D665AB0C42E58F1", data);//ä¸æ»¡8å­—èŠ‚
=======
//	len = MyStrToHex("3F12E7C02D665AB0C42E58F1", data);//²»Âú8×Ö½Ú
>>>>>>> add all file
